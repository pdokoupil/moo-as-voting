...
class marginal_gain_support_function:
    ...
    def __call__(self, item, context):
        ...
        if recsys_list is None: # TODO think about this (is this reasonable?)
            recsys_list = recommendation_list(context.k, [item])
            # Initialize cache
            self.cache_key = recsys_list.items[:]
            obj_value = self.objective(recsys_list, context)
        ...
        if hasattr(self.objective, "optimized_computation"):
            return self.objective.optimized_computation(recsys_list, item, context, self.cache_value, self.cache_extra_value) - self.cache_value
        
        # Return obj(list + item) - obj(list)
        return self._get_obj_value(self.objective(recsys_list.with_extra_item(item), context)) - self.cache_value
