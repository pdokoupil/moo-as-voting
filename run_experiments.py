...
LIGHTFM_TRAIN_FOLD_PATH = f"{cmd_args.dataset_path_prefix}/ml-100k-folds/lightfmfolds/0/train.dat"
LIGHTFM_TEST_FOLD_PATH = f"{cmd_args.dataset_path_prefix}/ml-100k-folds/lightfmfolds/0/test.dat"
...
def generate_latex_tables(all_results, plot_save_path_prefix):
    ...
    def calculate_per_user_kl_divergence(weights, rest_results):
        ...
        for method_name in rest_results.keys():
            ...
            assert len(per_user_mer) == len(per_user_diversity) == len(per_user_novelty), "All per-user results must have a same length"
            
            assert np.all(np.isfinite(np.array(per_user_mer))), f"MER must by finite: {per_user_mer}"
            assert np.all(np.isfinite(np.array(per_user_diversity))), f"Diversity must by finite: {per_user_mer}"
            assert np.all(np.isfinite(np.array(per_user_novelty))), f"Novelty must by finite: {per_user_mer}"
            
            # results[method_name] = [
            #     kl_divergence(weights, normalize_objective_values(globally_max_objective_values, np.array([mer, div, nov]))) for mer, div, nov in zip(per_user_mer, per_user_diversity, per_user_novelty)
            # ]
    ...
    for weights, rest_results in groupped_results.items():
        ...
        per_user_kl_divergence = calculate_per_user_kl_divergence(weights, rest_results)
...
"""  \\textbf{%s} &
  ...
  \\hline"""    % (
                    ...
                    results["map"], results["diversity"],
                    results["novelty"], results["mer"],
...
\\begin{tabular}{ | c | c | c | c | c | c | } 
  \\hline
  \\textit{Method} & \\textbf{MAP} & \\textbf{Diversity} & \\textbf{Novelty} & \\textbf{MER} & \\textbf{KL} \\\\ 
  \\hline
%s
